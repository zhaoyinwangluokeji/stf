  ...
  return {
    ...
    , link: function (scope, element) {
      ...
      scope.sortBy = function (column, multiple) {
        ...
        function findInSortingIndex(sorting) {
          for (var i = 0, l = sorting.length; i < l; ++i) {
            if (sorting[i].name === column.name) {
              return i
            }
      ...
      scope.$watch(
        ...
        , function (newValue) {
          activeSorting = newValue.user // newValue.user.concat(newValue.fixed)
      ...
      var compare = (function () {
        ...
        return function (deviceA, deviceB) {
          ...
          if (diff == 0) {
            for (var i = 0, l = scope.sort.fixed.length; i < l; ++i) {
              var sort = scope.sort.fixed[i]
              if (scope.columnDefinitions[sort.name] &&
                scope.columnDefinitions[sort.name].compare) {
                diff = scope.columnDefinitions[sort.name].compare(deviceA, deviceB)
                if (diff !== 0) {
                ...
                console.log("err sort name:" + sort.name)
              }
            }
          }
          return diff
      ...
      function sortAll() {
        ...
        var sorted = [].slice.call(rows).sort(function (rowA, rowB) {
          var ret = compare(mapping[rowA.id], mapping[rowB.id])
          return ret
        })
      ...
      function LoadData(startdate, enddate) {
        ...
        DeviceRentLogService.getLogs(condi, condi2, scope.page, scope.count, '', '').then(function (result) {
