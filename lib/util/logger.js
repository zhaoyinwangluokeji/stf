...
function IsNakedString(str) {
  str = str.toString()
  if (!!str) {
    var check = false
    for (const key in Logger.LevelLabel) {
      if (Logger.LevelLabel.hasOwnProperty(key)) {
        const element = Logger.LevelLabel[key];
...
process.stdout.write = function (d) {
  ...
  if (IsNakedString(d)) {
    ...
    fs.appendFileSync(logfile, d, function (res) {
    });
  }
};
