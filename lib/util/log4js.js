...
log4js.configure({
  appenders: {
    ...
    error_file: {//：记录器4：输出到error log
      type: "dateFile",
      filename: __dirname + `/../../logs/Error_Logs/${programName}_error`,//您要写入日志文件的路径
      alwaysIncludePattern: true,//（默认为false） - 将模式包含在当前日志文件的名称以及备份中
  ...
  categories: {
    ...
    produc: { appenders: ['data_file'], level: 'all' },  //生产环境 log类型 只输出到按日期命名的文件，且只输出警告以上的log
...
module.exports = log4js.getLogger('produc');
