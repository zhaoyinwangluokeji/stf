...
module.exports = syrup.serial()
  ...
  .define(function (options, solo, ident, service, router, push, sub, channels) {
    ...
    plugin.currentGroup = currentGroup
    plugin.join = function (newGroup, timeout, usage) {
      console.log("join:"+newGroup+",time:"+timeout+",usage:"+JSON.stringify(usage))
      return plugin.get()
        .then(function () {
        ...
        .catch(grouputil.NoGroupError, function () {
          ...
          console.log("NoGroupError:" + JSON.stringify(newGroup))
          log.important('Now owned by "%s"', currentGroup.email)
          log.info('Subscribing to group channel "%s",time s%', currentGroup.group, timeout || options.groupTimeout)
    ...
    plugin.leave2 = function (group, reason) {
      console.log("leave:" + CircularJSON.stringify(group))
      console.log("channels:" + CircularJSON.stringify(channels))
    ...
    plugin.leave = function (reason) {
      return plugin.get()
        .then(function (group) {
          console.log("leave:" + JSON.stringify(group))
          log.important('No longer owned by "%s"', group.email)
          log.info('Unsubscribing from group channel "%s"', group.group)
    ...
    router
      ...
      .on(wire.AutoGroupMessage, function (channel, message) {
        console.log("wire.AutoGroupMessage")
      ...
      .on(wire.UngroupMessage, function (channel, message) {
        console.log("wire.UngroupMessage")
        //  console.log("options.options:" + JSON.stringify(options))
        //  console.log("options.serial:" + channel)
          console.log("options.message:" + JSON.stringify(message))
