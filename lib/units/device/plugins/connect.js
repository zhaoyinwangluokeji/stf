...
module.exports = syrup.serial()
  ...
  .dependency(require('./rentcontrol'))
  .define(function (options, adb, router, push, group, solo, urlformat, rentcontrol) {
    ...
    plugin.start = function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        console.log("start device:"+plugin.url)
        var server = adb.createTcpUsbBridge(options.serial, {
          auth: function (key) {
            ...
            function notify() {
              group.get()
                .then(function (currentGroup) {
                  console.log("notify:" + currentGroup)
                  dbapi.dumpsUsersAdbKey(key.fingerprint).then(function () {
                    push.send([
                      solo.channel
            ...
            function notify1() {
              group.get()
                .then(function (currentGroup) {
                  console.log("notify1:" + currentGroup)
                  push.send([
                    solo.channel
                    , wireutil.envelope(new wire.JoinGroupByAdbFingerprintMessage(
                      options.serial
            ...
            router.on(wire.AdbKeysUpdatedMessage, notify1)
