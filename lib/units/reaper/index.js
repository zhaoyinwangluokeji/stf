  ...
  function DealEmail() {
    ...
    loadAllDevices().then(function (devicelist) {
      devicelist.forEach(device => {
        AllPromises.push(new Promise(function (resolve) {
          ...
          if (re > 0) {
            var subject
            if (re == 1) {
              subject = "Your rented cell phone:" + device.serial + " is about to expire,about 30 minutes"
            } else if (re == 2) {
              subject = "Your rented cell phone:" + device.serial + " has expired"
            }
      ...
      Promise.all(AllPromises).then(function (result) {
        ...
        console.log('Start DealEmail Timer2')
        timer = setTimeout(DealEmail, 60 * 1000)
      })
    })
  }
