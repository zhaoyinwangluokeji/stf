...
module.exports = function (options) {
    ...
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function () {
          ...
          .then(function () {
            log.info('add device: "%s"', JSON.stringify(req.body))
          ...
          .catch(function (err) {
            log.error('add device: Unexpected error', err.stack)
            res.status(500)
              .json({
                success: false
                , error: 'ServerError'
        ...
        requtil.validate(req, function () {
          ...
          .then(function () {
            ...
            dbapi.getCompatResultById(id).then(function (result) {
              log.info("compat result: " + JSON.stringify(result))
              res.status(200)
                .json({
                  success: true
                  , data: {
  ...
  app.post('/auth/api/v1/mock/GetAnnouncementList', function (req, res) {
    // console.log("post getGroup")
    var log = logger.createLogger('getGroup')
    log.setLocalIdentifier(req.ip)
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function () {
          ...
          .then(function () {
            ...
            var filter = (req.body.filter) ? (req.body.filter) : ""
            var total = 0
            dbapi.loadAnnouncementCount().then(function (ret) {
              total = ret
              dbapi.loadAnnouncement(page, count, filter).then(function (cursor) {
                return cursor.toArray()
              }).then(function (list) {
                res.status(200)
                  .json({
  ...
  app.post('/auth/api/v1/mock/InsertAnnouncement', function (req, res) {
    // console.log("post getGroup")
    var log = logger.createLogger('InsertAnnouncement')
    log.setLocalIdentifier(req.ip)
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function () {
          ...
          .then(function () {
            ...
            dbapi.InsertAnnouncement(announce).then(function () {
              res.status(200)
                .json({
                  success: true
                })
