...
module.exports = function (options) {
      ...
      case 'json':
        requtil.validate(req, function () {
          ...
          .then(function () {
            ...
            var loaduser1 = dbapi.saveDeviceGroupsInitialState(req.body.name, req.body.users, req.body.devices)
  ...
  app.post('/auth/api/v1/mock/get-all-device-groups', function (req, res) {
    ...
    var log = logger.createLogger('getDeviceGroups')
    log.setLocalIdentifier(req.ip)
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function () {
          ...
          .then(function () {
            dbapi.getAllDeviceGroups().then(function (cursor) {
              ...
              log.info("get device grups:" + JSON.stringify(list))
              res.status(200)
                .json({
                  success: true
                  , data: list
