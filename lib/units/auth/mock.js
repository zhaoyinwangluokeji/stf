          ...
          dbapi.loadGroupFName(groupname).then(function (result) {
            ...
            dbapi.SetUsersListOfGroup(groupname, userslist_s).then(function () {
              res.status(200)
                .json({
                  success: true
                  , data: 'success to add user' + JSON.stringify(userslist) + ' to group:' + groupname
                })
            })
          ...
          dbapi.loadGroupFName(groupname).then(function (result) {
            ...
            dbapi.SetUsersListOfGroup(groupname, newlist).then(function () {
              res.status(200)
                .json({
                  success: true
                  , data: 'success to remove user' + JSON.stringify(userslist) + 'from group:' + groupname
                })
            })
    ...
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function () {
          ...
          console.log("GetProgressBar:name=" + name + ",user=" + user)
