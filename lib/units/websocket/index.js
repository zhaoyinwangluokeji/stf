    ...
    var messageListener = wirerouter()
      ...
      .on(wire.LeaveGroupMessage, function (channel, message) {
        ...
        dbapi.loadDevice(message.serial).then(
          function (device) {
            var rentId = device.device_rent_conf.rentId
            dbapi.getDeviceUsingLog(rentId).then(function (ret) {
    ...
    new Promise(function (resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('device.admin_rent_conf.set', function (data) {
          ...
          data.device_rent_conf.now = Date.now()
          data.rentId = rentId;
          console.log("set device rent conf by admin:" + data.serial + ":" + JSON.stringify(data.device_rent_conf))
          var owner
          if (data.device_rent_conf.rent) {
            owner = data.device_rent_conf.owner
          } else {
        ...
        .on('device.user_rent_conf.set', function (data) {
          var rentId = util.format('%s-%s', uuid.v4(), uuid.v4()).replace(/-/g, '')
          if (data.device_rent_conf.rent == false) {
            if (data.device_rent_conf.rentId) {
              rentId = data.device_rent_conf.rentId;
        ...
        .on('get.predictimg', function (channel, responseChannel, data) {
          ...
          var req = http1.request(options, function (error, res, body) {
            if (error) {
              console.log("error:" + error) // Show the HTML for the baidu homepage.
              console.log("predict result: Fail 1...")
            ...
            res.on('end', function () {
              ...
              if (parseFloat(resultJSON["predictions"][0][0][0]) > 0.8) {
