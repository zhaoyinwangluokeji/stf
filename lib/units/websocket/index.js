        ...
        .on('device.rent_conf.set', function (channel, responseChannel, data) {
          try {
            ...
            dbapi.loadDevice(data.serial).then(function (dev) {
              ...
              else if (device.device_rent_conf && device.device_rent_conf.rent == true) {
                errormessage = '手机已经被他人租用:' + device.device_rent_conf.owner ? (device.device_rent_conf.owner.NameCN | device.device_rent_conf.owner.name) : "NoOwner"
              }
              if (errormessage != "") {
                socket.emit('tx.done', responseChannel.toString(), new wire.TransactionDoneMessage(
                  channel
