...
module.exports = function(options) {
  ...
  io.on('connection', function(socket) {
    ...
    var messageListener = wirerouter()
      ...
      //  console.log('device.add:'+channel)
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('device.rent_conf.set', function(data) {
          data.deivce_rent_conf.start_time = Date.now()
          data.deivce_rent_conf.now =  Date.now()
          return dbapi.setDeviceRentConf(data.serial, data.deivce_rent_conf)
            .then(function() {
              return dbapi.loadDevice(data.serial)
            })
            .then(function(device) {
              if (device) {
                ...
                , data: {
                    serial: device.serial
                  , notes: device.notes
                  ,deivce_rent_conf:data.deivce_rent_conf
                  }
                })
                ...
                , wireutil.envelope(new wire.DeviceRentMessage(
                  device.serial,
                  data.deivce_rent_conf.rent,
                  data.deivce_rent_conf.rent_time,
                  Date.now()  
                ))
                ])
        ...
        .on('get.verifycode', function(channel, responseChannel) {
          joinChannel("N8K7N17206007791")
          var reply = wireutil.reply("verify...")
          console.log("sending..." + reply.progress("sending...123456"))
