...
module.exports = function () {
  ...
  function writeInstallResult(id, serial, manufacturer, model, platformVersion, installResult, installErrorMsg, launchResult, launchTime, currentState, installTime, hotLuanchTime = null) {
    var resultData = {
      serial: serial,
            ...
            executeShellAndGetResult(serial, 'am start -W ' + launchActivity.component)
              .then(function (result) {
                ...
                if (result.result == 'fail') {
                  writeInstallResult(id, serial, manufacturer, model, platformVersion, 'success', null, 'fail', 'unknown', 'failed', installTime)
                ...
                log.info('start activity result: ' + launchTime)
                if (launchTime == 'unknown') {
                  return writeInstallResult(id, serial, manufacturer, model, platformVersion, 'success', null, 'fail', 'unknown', 'failed', installTime)
                } else {
                    ...
                    setTimeout(() => {
                      ...
                      executeShellAndGetResult(serial, 'am start -W ' + launchActivity.component)
                        .then(function (result) {
                          ...
                          if (result.result == 'fail') {
                            writeInstallResult(id, serial, manufacturer, model, platformVersion, 'success', null, 'fail', launchTime, 'failed', installTime, 'unknown')
                            return resolver.resolve()
                          }
                          var tmpResult = result.data.split('\n')
                          for (i = 0; i < tmpResult.length; i++) {
                          ...
                          log.info('hot start activity result: ' + hotLaunchTime)
                          if (hotLaunchTime == 'unknown') {
                            return writeInstallResult(id, serial, manufacturer, model, platformVersion, 'success', null, 'fail', launchTime, 'failed', installTime, hotLaunchTime)
                          } else {
