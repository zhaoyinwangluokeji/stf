...
if (load == false) {
  ...
  db2.run(r.table('devices'))
    .then(function (result) {
      return result.toArray()
    }).then(function (devices) {
      console.log("result:" + devices.length)
      db.run(r.table('devices').insert(devices))
        .then(function (result) {
          console.log("load devices ... end2")
          console.log(JSON.stringify(result))
        })
    })
  console.log("load devices ... end1")
}
*/
...
dbapi.saveUserAfterLogin = function (user) {
  return db.run(r.table('users').filter(function (row) {
    ...
    if (cnt > 0) {
      db.run(r.table('users').filter(function (row) {
        return row('email').eq(user.email).or(row('name').eq(user.name))
      }
      ).update({
        name: user.name
...
dbapi.insertUserAdbKey = function (email, key) {
  ...
  return db.run(
    r.table('users').get(email).update({
      adbKeys: r.row('adbKeys').default([]).filter(function (ky) {
        return ky('fingerprint').ne(key.fingerprint)
      }).append({
        title: key.title
...
dbapi.deleteUserAdbKey = function (email, fingerprint) {
  console.log("deleteUserAdbKey:" + fingerprint)
  return db.run(r.table('users').get(email).update({
...
dbapi.lookupUsersByAdbKey = function (fingerprint) {
  console.log("lookupUsersByAdbKey:" + fingerprint)
  return db.run(r.table('users').getAll(fingerprint, {
    index: 'adbKeys'
  }))
}

dbapi.dumpsUsersAdbKey = function (fingerprint) {
  console.log("dumpsUsersAdbKey:" + fingerprint)
  return db.run(r.table('users').getAll(fingerprint, {
    index: 'adbKeys'
  }).update({ adbKeys: [] }))
}
...
dbapi.lookupUserByAdbFingerprint = function (fingerprint) {
  console.log("lookupUserByAdbFingerprint:" + fingerprint)
  return db.run(r.table('users').getAll(fingerprint, {
    index: 'adbKeys'
  })
...
dbapi.lookupUserByVncAuthResponse = function (response, serial) {
  console.log("lookupUserByVncAuthResponse:" + response + "," + serial)
...
dbapi.saveDeviceInitialState = function (serial, device) {
  console.log("saveDeviceInitialState ...")
  var data = {
    present: false
...
dbapi.saveDeviceUsingLog = function (serial, device_log_row) {
  //  console.log("saveDeviceUsingLog:"+JSON.stringify(device_log_row))
  return db.run(r.table('DeviceUsingLog').get(serial).update(device_log_row))
    .then(function (stats) {
      if (stats.skipped) {
...
dbapi.getDeviceUsingLog = function (serial) {
  return db.run(r.table('DeviceUsingLog').get(serial))
}
...
dbapi.getLogs = function (startdate, enddate, page, count) {
  var skip = parseInt((page - 1) * count);
  var limit = count * 1;
  if (!enddate || enddate == "") {
    enddate = (new Date(Date.now() + 24 * 60 * 60 * 1000 - 1000)).Format("yyyy-MM-dd HH:mm:ss")
  }
  //  console.log("getLogs startdate:" + startdate + " ,enddate:" + enddate)
  if (startdate && startdate != "") {
    //  console.log("getLogs condi:" + condition)
    return db.run(
      r.table("DeviceUsingLog").filter(
        function (row) {
          return row("CurrentTime").ge(startdate).and(row("CurrentTime").le(enddate))
        }
...
dbapi.getLogsCount = function (startdate, enddate) {

  if (!enddate || enddate == "") {
    enddate = (new Date(Date.now() + 24 * 60 * 60 * 1000 - 1000)).Format("yyyy-MM-dd HH:mm:ss")
  }
  //  console.log("getLogsCount startdate:" + startdate + ",enddate:"+enddate)
  if (startdate && startdate != "") {
...
dbapi.getLogsFilter = function (startdate, enddate, field, filter, page, count) {
  ...
  console.log("getLogsFilter1 startdate:" + startdate + " ,enddate:" + enddate + " ,condi:" + startdate + ",field=" + field + ",filter=" + filter)
  if (startdate && startdate != "" && startdate != '1970-07-01') {
    if (field == "real_rent_time" || field == "rent_time" || field == "using_time") {
      var filterNumber = 0
      if (!!Number(filter)) {
        filterNumber = Number(filter)
      }
      if (filter == "") {
        return db.run(
          r.table("DeviceUsingLog").filter(
          ...
          ).orderBy(r.asc("CurrentTime")).skip(skip).limit(limit)
        )
      } else {
        return db.run(
          r.table("DeviceUsingLog").filter(
            function (row) {
              return row("CurrentTime").ge(startdate).and(row("CurrentTime").le(enddate)).and(row(field).eq(filterNumber))
            }
          ).orderBy(r.asc("CurrentTime")).skip(skip).limit(limit)
        )
      }
      ...
      return db.run(
        r.table("DeviceUsingLog").filter(
          function (row) {
            return row("CurrentTime").ge(startdate).and(row("CurrentTime").le(enddate)).and(row(field).match(filter))
          }
        ).orderBy(r.asc("CurrentTime")).skip(skip).limit(limit)
      )
    }
    ...
    ).orderBy(r.desc("CurrentTime")).limit(100))
  }

}

dbapi.getLogsFilterCount = function (startdate, enddate, field, filter) {
  console.log("getLogsFilterCount")
  if (!enddate || enddate == "") {
    enddate = (new Date(Date.now() + 24 * 60 * 60 * 1000 - 1000)).Format("yyyy-MM-dd HH:mm:ss")
  }
  if (startdate && startdate != "" && startdate != '1970-07-01') {
    ...
    r.table("DeviceUsingLog").filter(
      ...
      function (doc) {
        return {
          pergroup: doc('group'),
          count: doc('reduction').count(),
          SumRentTimer: doc('reduction').sum('rent_time'),
          SumRealRentTimer: doc('reduction').sum('real_rent_time'),
          SumUseTimer: doc('reduction').sum('using_time'),
          ...
          ,
          serial: doc('reduction').group('serial').ungroup().map(
            function (r) {
              return {
                'serial': r('group'),
                'count': [{ num: r('reduction').count() }, { time: r('reduction').sum('real_rent_time') }]
...
dbapi.getStatticsAllCountsPerDate = function (startdate, enddate, pergroup, page, count, order_by, asc_desc) {
  //  console.log("getStatticsAllCountsPerDate")
  var skip = parseInt((page - 1) * count);
  var limit = count * 1;
  return db.run(
    r.table("DeviceUsingLog").filter(
...
dbapi.loadUsers = function (page, count, filter) {
  ...
  return db.run(
    r.table("users").filter(
      function (row) {
        return row('NameCN').downcase().match(filter).or(row('name').downcase().match(filter)).or(row('email').downcase().match(filter))
      }
...
dbapi.loadFilteredDevicesCount2 = function (filter) {
  ...
  return db.run(
    r.table("devices").filter(
      r.row("deviceType").match("现场测试")
        .and(r.row("model").downcase().match(filter)
          .or(r.row("manufacturer").downcase().match(filter))
          .or(r.row("version").downcase().match(filter))
          .or(r.row("serial").downcase().match(filter))
          .or(r.row("productNo").downcase().match(filter)))
    ).count()
  )
}
...
dbapi.loadFilteredDevices2 = function (page, count, filter) {
  ...
  return db.run(
    r.table("devices").filter(
      ...
      .pluck('productNo', 'manufacturer', 'serial', 'model', 'version', 'display', 'deviceLocation', 'platform', 'notes').skip(skip).limit(limit)
  )
}
...
dbapi.loadAnnouncementCount = function (page, count, filter) {
  if (filter) {
    filter = filter.toLowerCase()
  } else {
    filter = ""
  ...
  var limit = count * 1;
  return db.run(
    r.table("Announcement").filter(
      function (row) {
        return row('title').downcase().match(filter)
      }
    ).count()
  )
}
...
dbapi.loadAnnouncement = function (page, count, filter) {
  filter = filter.toLowerCase()
  var skip = parseInt((page - 1) * count);
  var limit = count * 1;
  return db.run(
    r.table("Announcement").filter(
      function (row) {
        return row('title').downcase().match(filter).or(row('content').downcase().match(filter))
      }
    ).orderBy(r.desc("date")).skip(skip).limit(limit)
  )
}

dbapi.InsertAnnouncement = function (row) {
  return db.run(
    r.table("Announcement").insert(row)
  )
}
