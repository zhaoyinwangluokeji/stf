...
dbapi.deleteExistsUser = function (useremails, usernames) {
...
dbapi.loadUsersCount = function (filter) {
  return db.run(
    r.table("users").filter(
      function (row) {
        return row('email').match(filter).or(
          row('name').match(filter)
        )
      }
    ).count()
  )
...
dbapi.loadUsers = function (page, count, filter) {
  ...
  return db.run(
    r.table("users").filter(
      function (row) {
        return row('email').match(filter).or(row('name').match(filter))
      }
    ).pluck('email', 'name').skip(skip).limit(limit)
  )
}
...
dbapi.loadPermissionCount = function (filter) {
  return db.run(
    r.table("PermissionTable").filter(
      function (row) {
        return row('desc').match(filter).or(
          row('name').match(filter)
        )
      }
    ).count()
...
dbapi.loadAllPermission = function (page, count, filter) {
  ...
  return db.run(
    r.table("PermissionTable").filter(
      function (row) {
        return row('desc').match(filter).or(row('name').match(filter))
      }
    ).skip(skip).limit(limit)
  )
}
  ...
  return db.run(
    r.table("devices").filter(
      r.row("model").match(filter)
        .or(r.row("version").match(filter))
        .or(r.row("serial").match(filter))
    ).count()
  )
}
...
dbapi.loadFilteredDevicesCount2 = function (filter) {
  return db.run(
    r.table("devices").filter(
      r.row("model").match(filter)
        .or(r.row("manufacturer").match(filter))
        .or(r.row("platform").match(filter))
        .or(r.row("version").match(filter))
        .or(r.row("serial").match(filter))
    ).count()
  )
...
dbapi.loadUserGroupCount = function (filter) {
  return db.run(
    r.table("UserGroup").filter(
      function (row) {
        return row('GroupName').match(filter)
      }
    ).count()
  )
}
