...
dbapi.setDeviceRentConf = function(serial, deivce_rent_config) {
  ...
  return db.run(r.table('devices').get(serial).update({
    deivce_rent_conf: deivce_rent_config
  }))
}
...
dbapi.loadProjectsByProjectName = function(projectName) {
  ...
  return db.run(r.table('ProjectInfo')
      .filter(r.row("ProjectName").match("^"+projectName)).pluck("ProjectCode","ProjectName").limit(50))
  
}
dbapi.loadProjectsByProjectId= function(projectId) {
  if(!projectId) projectId = ""
  return db.run(r.table('ProjectInfo')
  .filter(r.row("ProjectId").match("^"+ProjectId)).pluck("ProjectCode","ProjectName").limit(50))
}
dbapi.loadProjectsByProjectCode= function(projectCode) {
  if(!projectCode) projectCode = ""
  return db.run(r.table('ProjectInfo')
  .filter(r.row("ProjectCode").match("^"+projectCode)).pluck("ProjectCode","ProjectName").limit(50))
}
dbapi.loadProjectsByUser= function(user) {
  if(!user) user = ""
  return db.run(r.table('ProjectInfo')
  .filter(r.row("PM").match("^"+user)).pluck("ProjectCode","ProjectName").limit(50))
}
dbapi.loadProjectsByST = function(st_user) {
  if(!st_user) st_user = ""
  return db.run(r.table('ProjectInfo')
  .filter(r.row("ST_PM").match("^"+st_user)).pluck("ProjectCode","ProjectName").limit(50))
}
dbapi.loadProjectsByUAT = function(uat_user) {
  if(!uat_user) uat_user = ""
  return db.run(r.table('ProjectInfo')
  .filter(r.row("UAT_PM").match("^"+uat_user)).pluck("ProjectCode","ProjectName").limit(50))
}
