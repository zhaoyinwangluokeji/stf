...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('compat-url', {
      alias: 'a'
    , describe: 'URL to the compat unit.'
    , type: 'string'
    })
    .option('compat-port', {
      describe: 'The port the compat unit should run at.'
    , type: 'number'
    , default: 7107
    })
    .option('storage-type', {
...
module.exports.handler = function(argv) {
  ...
  function run() {
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--compat-url'
        , argv.compatUrl || util.format('http://localhost:%d/', argv.compatPort)
        , '--storage-url'
  ...
  /*  , procutil.fork(path.resolve(__dirname, '..'), [
    ...
    // 如果没有设置--compat-url，则本地启动一个。
    if (!argv.compatUrl) {
      procs.push(
        // compat
        procutil.fork(path.resolve(__dirname, '..'), [
          'compat'
        , '--port', argv.compatPort
        , '--storage-url'
        , util.format('http://localhost:%d/', argv.storagePort)
        ])
        )
